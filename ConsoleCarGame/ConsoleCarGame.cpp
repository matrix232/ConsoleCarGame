#include <iostream>
#include <time.h> // For manipulation wuth time
#include <conio.h> // For work with console input\output like keyboard controller
#include <windows.h> // Wor work with window API
#include <stdlib.h>
#include <string>
...
void startUp()
{
    /*all values once the game start*/
    srand(time(NULL));
    exitGame = false;
    isMoving = false;
    gameOver = false;
...
void input()
{
    /*to control your car using keyboard input*/
    if (_kbhit())
    {
        switch (_getch())
        {
...
void died()
{
    /*logic when ypur character crash*/
    if (dead)
    {
        life--;
        int count = 0;
